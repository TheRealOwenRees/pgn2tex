const pgn2tex = require('../pgn2tex');

// Test TeX output from the below PGN and object of diagrams
...
// Manual tests - prints TeX to console instead
// const res = pgn2tex(pgn, diagrams);
// console.log(res);

describe('Valid TeX Output', () => {
  test('beginning and end forms valid TeX', () => {
    const texOutput = pgn2tex(pgn, diagrams);
    expect(texOutput.startsWith('\\documentclass{article}\\usepackage{xskak}\\usepackage{multicol}\\usepackage[a4paper]{geometry}\\usepackage{parskip}\\geometry{left=1.25cm,right=1.25cm,top=1.5cm,bottom=1.5cm,columnsep=1.2cm}\\setlength{\\parindent}{0pt}\\title')).toBe(true);
    expect(texOutput.endsWith('\\end{multicols}\\end{document}')).toBe(true);
  });
});
